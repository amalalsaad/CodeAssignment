...
class SimpleNeuralNetwork:
    ...
    def backward(self, Y: torch.Tensor, learning_rate: float = 0.01) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        # Output layer gradient (assuming mean squared error loss)
        ...
        dA2 = torch.matmul(dZ3, self.W3)  # (batch_size, hidden_size2)
        dZ2 = dA2 * (self.Z2 > 0).float()  # Applying ReLU derivative
        ...
        dA1 = torch.matmul(dZ2, self.W2)  # (batch_size, hidden_size1)
        dZ1 = dA1 * (self.Z1 > 0).float()  # Applying ReLU derivative
    ...
    def backward(self, Y: torch.Tensor, learning_rate: float = 0.01) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        dX = torch.matmul(dZ1, self.W1)  # Gradient with respect to input X
        ...
        # Return the tuple of specified gradients
